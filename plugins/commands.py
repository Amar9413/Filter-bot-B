...
async def start(client:Client, message): 
            ...
            if refData and refData.split("-", 1)[0] == "biisal":
                ...
                if AUTH_CHANNEL and await is_req_subscribed(client, message):
                        buttons = [[
                            ...
                            [
                            InlineKeyboardButton('🎁 ɢᴇᴛ ᴘʀᴇᴍɪᴜᴍ ꜰʀᴇᴇ 🎁', callback_data=f'free_premium#{message.from_user.id}'),
                            InlineKeyboardButton("ᴜᴘᴅᴀᴛᴇᴅ ᴄʜᴀɴɴᴇʟ😊", url='https://t.me/+5kjfpYLVM25hZGNl')
                            ],
                            [
                            InlineKeyboardButton('🎭 ʏᴏᴜʀ ᴘᴏɪɴᴛ ✨', callback_data=f'point#{message.from_user.id}'),
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener or is_third_shortener:
            ...
            verify = await get_shortlink(f"https://telegram.me/Dawnload_Hub_Bot?start=notcopy_{user_id}_{verify_id}_{file_id}" , grp_id, is_second_shortener, is_third_shortener , pm_mode=pm_mode)
            buttons = [[
...
async def save_tutorial(client, message):
    ...
    try:
        ...
        return await message.reply_text("<b>Command Incomplete!!\n\nuse like this -</b>\n\n<code>/set_caption https://t.me/MovieDawnloadHub</code>")    
...
async def set_shortner(c, m):
    ...
    if len(m.text.split()) == 1:
        await m.reply("<b>Use this command like this - \n\n`/set_shortner mdiskshortner.link 88f79a7365f4723412aef3e2b3f602dd7fd901c9`</b>")
        return        
    ...
    try:
        URL = m.command[1]
        API = m.command[2]
        resp = requests.get(f'https://{URL}/api?api={API}&url=https://t.me/MovieDawnloadHub').json()
...
async def set_shortner_2(c, m):
    ...
    if not await is_check_admin(c, grp_id, m.from_user.id):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    if len(m.text.split()) == 1:
        await m.reply("<b>Use this command like this - \n\n`/set_shortner_2 mdiskshortner.link 88f79a7365f4723412aef3e2b3f602dd7fd901c9`</b>")
        return
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await asyncio.sleep(1.2)
    await sts.delete()
    try:
        ...
        await m.reply_text(f"<b><u>💢 ᴇʀʀᴏʀ ᴏᴄᴄᴏᴜʀᴇᴅ!!</u>\n\nᴀᴜᴛᴏ ᴀᴅᴅᴇᴅ ʙᴏᴛ ᴏᴡɴᴇʀ ᴅᴇꜰᴜʟᴛ sʜᴏʀᴛɴᴇʀ\n\nɪꜰ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ ᴄʜᴀɴɢᴇ ᴛʜᴇɴ ᴜsᴇ ᴄᴏʀʀᴇᴄᴛ ꜰᴏʀᴍᴀᴛ ᴏʀ ᴀᴅᴅ ᴠᴀʟɪᴅ sʜᴏʀᴛʟɪɴᴋ ᴅᴏᴍᴀɪɴ ɴᴀᴍᴇ & ᴀᴘɪ\n\nʏᴏᴜ ᴄᴀɴ ᴀʟsᴏ ᴄᴏɴᴛᴀᴄᴛ ᴏᴜʀ <a href=https://t.me/MovieDawnloadHub_Tutorial>sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ</a> ꜰᴏʀ sᴏʟᴠᴇ ᴛʜɪs ɪssᴜᴇ...\n\nʟɪᴋᴇ -\n\n`/set_shortner_2 mdiskshortner.link 88f79a7365f4723412aef3e2b3f602dd7fd901c9`\n\n💔 ᴇʀʀᴏʀ - <code>{e}</code></b>", quote=True)
...
async def set_shortner_3(c, m):
    ...
    if len(m.text.split()) == 1:
        return await m.reply("<b>Use this command like this - \n\n`/set_shortner_3 mdiskshortner.link 88f79a7365f4723412aef3e2b3f602dd7fd901c9`</b>")
    sts = await m.reply("<b>♻️ ᴄʜᴇᴄᴋɪɴɢ...</b>")
    await sts.delete()
    ...
    if not await is_check_admin(c, grp_id, userid):
        return await m.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
    if len(m.command) == 1:
        await m.reply_text("<b>ᴜsᴇ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ᴀᴅᴅ sʜᴏʀᴛɴᴇʀ & ᴀᴘɪ\n\nᴇx - `/set_shortner_3 mdiskshortner.link 88f79a7365f4723412aef3e2b3f602dd7fd901c9`</b>", quote=True)
        return
    try:
        URL = m.command[1]
        API = m.command[2]
